...
def pre_process(raw_text):
    ...
    # remove urls
    url_pattern = 'http'
    url_removed_text = re.sub(url_pattern, '', str(url_removed_text))
    ...
    # remove mentions
    #user_mention_removed_text = re.sub('@\S+', '', hashtag_removed_text)

    # keep only words
    letters_only_text = re.sub("[^a-zA-Z]", " ", str(url_removed_text))
